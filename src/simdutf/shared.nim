...
func simdutfResult*(res: simdutf_result): SimdutfResult {.inline.} =
  ...
  native.error =
    ...
    of error_too_large:
      TooLarge
    of error_invalid_base64_character:
      InvalidBase64Character
    of error_base64_input_remainder:
      BaseInputRemainder
    of error_output_buffer_too_small:
      OutputBufferTooSmall
    of error_other:
      Other
    else:
      assert false, "Invalid simdutf_result error code: " & $res.error
      Success
...
func resultToString*(res: simdutf_result): string {.inline.} =
  ...
  of error_success:
    "Success"
  of error_header_bits:
    "Byte at position $1 has more than 5 header bits!" % [$res.count]
  of error_too_short:
    "At position $1; the leading byte must be followed by N-1 continuation bytes, where N is the continuation UTF-8 character length." %
      [$res.count]
  of error_too_long:
    if res.count == 0:
      "Input cannot start with a continuation byte!"
    else:
      "At position $1; too many consecutive continuation bytes were found!" %
        [$res.count]
  of error_surrogate:
    "At position $1; this character is either in U+D800..DFFF. If it is a high surrogate, it must be followed by a low surrogate and that low surrogate must be preceded by a high surrogate (this character) or no surrogate at all (Latin1)." %
      [$res.count]
  of error_invalid_base64_character:
    "At position $1; invalid character detected whilst decoding base64-encoded string!" %
      [$res.count]
  of error_base64_input_remainder:
    "This base64-encoded string ends with a single character, excluding the padding symbols."
  of error_output_buffer_too_small:
    "The output buffer provided is too small to fit the entire decoded segment!"
  of error_other:
    "An error occured which is not related to validation or transcoding."
  else:
    "Unhandled error code provided (" & $res.error & ')'
